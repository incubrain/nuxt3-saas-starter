<template>
  <!-- Navigation starts -->
  <nav
    class="flex items-center md:items-stretch w-full justify-end md:justify-between foreground shadow relative z-[0] px-4 sm:h-[var(--nav-height-sm)] md:h-[var(--nav-height-md)] lg:h-[var(--nav-height-lg)]"
  >
    <div
    ...
    >
      ...
      <div class="h-full flex items-center col-start-1 col-span-2 md:col-span-1">
        ...
        <div
          v-show="route.path === '/'"
          class="hidden lg:flex gap-4 pl-4 justify-center border-l items-center h-full leading-none text-sm whitespace-nowrap"
        >
          <NuxtLink to="/users" class="nav-link"> Features </NuxtLink>
          <NuxtLink to="/about" class="nav-link"> About </NuxtLink>
          <NuxtLink to="/contact" class="nav-link"> Pricing </NuxtLink>
        </div>
      </div>
